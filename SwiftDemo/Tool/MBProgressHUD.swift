...
func MB_TEXTSIZE(_ text: String?, font: UIFont) -> CGSize {
    guard let textTemp = text, String(textTemp).count > 0 else {
        return CGSize.zero
    }
...
class MBProgressHUD: UIView {
    ...
    var square = false
    var oldSize: CGSize = CGSize.zero
    ...
    override func draw(_ rect: CGRect) {
        ...
        let boxRect: CGRect = CGRect(x: round((allRect.size.width - oldSize.width) / 2) + CGFloat(self.xOffset), y: round((allRect.size.height - size.height) / 2) + CGFloat(self.yOffset), width: size.width, height: size.height)
