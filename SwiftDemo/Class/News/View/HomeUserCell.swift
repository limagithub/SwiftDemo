    ...
    var aNews = NewsModel() {
        didSet {
            ...
            if aNews.source != "" { nameLabel.text = aNews.source }
                ...
                avatarImageView.sd_setImage(with: URL.init(string: aNews.user.avatar_url.suffixWebpToPng())!, completed: nil);
                nameLabel.text = aNews.user.screen_name
            } else if aNews.user_info.user_id != 0 {
                avatarImageView.sd_setImage(with: URL.init(string: aNews.user_info.avatar_url.suffixWebpToPng())!, completed: nil);

                nameLabel.text = aNews.user_info.name
            } else if aNews.media_info.user_id != 0 {
                nameLabel.text = aNews.media_info.name
                avatarImageView.sd_setImage(with: URL.init(string: aNews.media_info.avatar_url.suffixWebpToPng())!, completed: nil);
       
            }
            readCountLabel.text = "\(aNews.readCount)阅读"
            verifiedContentLabel.text = aNews.verified_content
            digButton.setTitle(aNews.digg_count, for: .normal)
            commentButton.setTitle("\(aNews.comment_count)", for: .normal)
            feedshareButton.setTitle(aNews.forward_count, for: .normal)
