...
class ThreeImageCell: UITableViewCell {
    ...
    @IBOutlet weak var spaceLabel: NSLayoutConstraint!
    ...
    var aNews = NewsModel() {
        didSet {
            ...
            if aNews.image_list.count == 1 {
                firstImage.sd_setImage(with: URL.init(string: (aNews.image_list.first?.url.suffixWebpToPng())!)!, completed: nil);
            } else if aNews.image_list.count == 2 {
                firstImage.sd_setImage(with: URL.init(string: (aNews.image_list.first?.url.suffixWebpToPng())!)!, completed: nil);
                secondImage.sd_setImage(with: URL.init(string: (aNews.image_list[1].url.suffixWebpToPng()))!, completed: nil);
            } else if aNews.image_list.count == 3 {
                firstImage.sd_setImage(with: URL.init(string: (aNews.image_list.first?.url.suffixWebpToPng())!)!, completed: nil);
                secondImage.sd_setImage(with: URL.init(string: (aNews.image_list[1].url.suffixWebpToPng()))!, completed: nil);
                threeImage.sd_setImage(with: URL.init(string: (aNews.image_list[2].url.suffixWebpToPng()))!, completed: nil);
            }
        }
    }
